...
const CHART_PEERS = ['stori', 'nu', 'klar', 'revolut', 'uala', 'didi'];

const NAV = [
  { id: 'overview', label: 'Overview', icon: 'grid' },
  { id: 'metrics', label: 'Peer Metrics', icon: 'table' },
  { id: 'arpoc', label: 'Revenue / Customer', icon: 'line' },
  { id: 'gross', label: 'Gross Profit', icon: 'bars' },
  { id: 'deposits', label: 'Deposit Products', icon: 'layers' },
...
function MonthRange({ months, range, onChange }) {
  return (
    <div className="mrange">
      <span className="mrange-lbl">Period</span>
...
function YoYPanel({ data, peers, visible }) {
  const keys = CHART_PEERS.filter((k) => visible[k] && (k in data.yoy));
  return (
    <div className="yoy-panel">
      <div className="yoy-head">YoY Growth %</div>
...
function KpiCards({ data, peers, order }) {
  const picks = ['portfolio', 'netincome', 'arpoc', 'margin', 'revgrowth', 'nim'];
  return (
...
/* --------------------------- TIME-SERIES SECTION ------------------------- */
function TrendSection({ ds, title, sub, defaultType, peers, months, visible, onToggle, theme }) {
    ...
    <div className="section-body">
      ...
      <div className="chart-row">
        <div className="chart-card">
          ...
          <Chart data={{ series: sliced.series }} peers={peers} labels={sliced.labs} visible={visible} showLabels={labels} theme={theme} />
        </div>
        <YoYPanel data={ds} peers={peers} visible={visible} />
      </div>
    </div>
  );
}
...
function App() {
  ...
  function renderSection(id) {
    switch (id) {
      case 'overview':
        return (
          <Section key={id} id={id} title="Overview" sub="Stori vs. peer set — latest reported figures" registerRef={registerRef}>
            <KpiCards data={CI.METRICS} peers={CI.PEERS} order={CI.METRIC_ORDER} />
            <div className="callout">
              <span className="callout-mark" />
              <div><strong>Stori delivers stronger profits as credit quality continues to recover.</strong>
                Highest Gross ROA and Risk-Adjusted NIM in the peer set, with positive net income while most challengers remain in the red.</div>
            </div>
          </Section>
        );
      case 'metrics':
        return (
          <Section key={id} id={id} title="Peer Metrics Comparison" sub="Click any metric to rank peers · Stori highlighted" registerRef={registerRef}>
            <div className="chart-toolbar">
              <PeerChips peers={CI.PEERS} visible={visible} onToggle={toggle} keys={CI.METRIC_ORDER.filter((k) => k !== 'plata')} />
              <div className="ct-right">
            ...
            <div className="card">
              <MetricsTable data={CI.METRICS} peers={CI.PEERS} order={CI.METRIC_ORDER}
                visible={{ ...visible, plata: visible.plata ?? true }} highlightStori={highlightStori} heatmap={heatmap} />
            </div>
      ...
      case 'deposits':
        return (
          <Section key={id} id={id} title="Deposit Product Comparison" sub="Rates and feature coverage across the deposit landscape" registerRef={registerRef}>
            <div className="card"><DepositTable cols={CI.DEPOSIT_COLS} rows={CI.DEPOSIT_ROWS} theme={theme} /></div>
          </Section>
        );
      case 'funding':
        return (
            ...
            <div className="card cap-card">
              <div className="cap-legend">
                {CI.CAP_STRUCTURE.layers.map((l) => (
                  <span key={l.key} className="cap-leg-item"><span className="cap-leg-dot" style={{ background: l.color }} />{l.label}</span>
                ))}
      ...
      <div className="content">
        <header className="topbar">
          <div className="topbar-title">
            ...
            <span className="period-pill">As of Mar 2026</span>
          </div>
